...
create_container() {
    ...
    read -p "Enter data folder or leave empty for '$(pwd)/agent-flux': " data_folder
    data_folder=${data_folder:-$(pwd)/agent-flux}
...
# Main program loop
echo -e "\nWelcome to Agent Flux Docker Management Script.\n"
while true; do
    list_containers
    if [ $? -ne 0 ]; then
