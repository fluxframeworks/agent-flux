...
With proper instruction, Agent Flux is capable of many things, even potentially dangerous to your computer, data, or accounts. Always run Agent Flux in an isolated environment, preferably in the built-in automated Docker solution.
...
## Ideal environment
- **Linux VM / docker container**: The perfect environment to run Agent Flux is a Debian-based Linux virtual machine or (event better) the built-in docker container with enhanced privileges or root access (to install packages and run terminal commands).
